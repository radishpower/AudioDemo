...
public class SoundBox {
		...
		Track(String filename) throws IOException {
			...
			track = new AudioTrack(
					...
					wave.bitsPerSample == 8 ? AudioFormat.ENCODING_PCM_8BIT
							: AudioFormat.ENCODING_PCM_16BIT, wave.data.length,
	...
	/**
	 * load a clip from the given filename and store it Note: executes the task
	 * at creation time use .id to later refer to the loaded clip
	 */
	 ...
	 * simply ignore all requests (below) if the clip is not there yet (or
	 * failed to load).
	 * 
	 * @param filename
	 * @return clip id
